...
export default function TabLayout() {
  ...
  useEffect(() => {
    console.log("[Tab Layout] User role changed:", userRole);
    console.log("[Tab Layout] Auth loading:", authLoading);
    console.log("[Tab Layout] User:", user?.id);
    ...
    console.log("[Tab Layout] Is organizer or admin:", isOrganizerOrAdmin);
    
    if (isOrganizerOrAdmin) {
      console.log("[Tab Layout] Adding organizer tab");
      baseTabs.splice(1, 0, {
        name: 'organizer',
    ...
    console.log("[Tab Layout] Final tabs:", baseTabs.map(t => t.label).join(', '));
    setTabs(baseTabs);

...
const isAbortLikeMessage = (message: string): boolean => {
  ...
  // Abort/cancellation is common in RN/Supabase when navigating away or when a timeout aborts.
  // Treat these as non-actionable noise for dev log forwarding.
  return (
    m.includes('aborterror') ||
    m.includes('request was aborted') ||
    m.includes('aborted (timeout') ||
    m.includes('timeout or manual cancellation') ||
    m.includes('the operation was aborted') ||
    m.includes('signal is aborted') ||
    // Broad fallback: many libs only include "aborted".
    m.includes('aborted')
  );
};
...
const shouldMuteMessage = (message: string): boolean => {
  if (isAbortLikeMessage(message)) return true;
